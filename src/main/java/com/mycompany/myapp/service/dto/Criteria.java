...
public abstract class Criteria<ENTITY> {
    ...
    protected <OTHER, X> Specification<ENTITY> buildReferringEntitySpecification(
        Criteria<OTHER> otherCriteria,
        SingularAttribute<ENTITY, X> field,
        Class<OTHER> otherClass
    ) {
        return (root, query, criteriaBuilder) -> {
            Root<OTHER> otherRoot = query.from(otherClass);
            return criteriaBuilder.and(
                criteriaBuilder.equal(otherRoot, root.get(field)),
                otherCriteria.toSpecification().toPredicate(otherRoot, query, criteriaBuilder)
            );
        };
    }

    protected <OTHER> Specification<ENTITY> buildReferringEntitySpecification(
        Criteria<OTHER> otherCriteria,
        SetAttribute<ENTITY, OTHER> field,
        Class<OTHER> otherClass
    ) {
        return (root, query, criteriaBuilder) -> {
            Root<OTHER> otherRoot = query.from(otherClass);
            return criteriaBuilder.and(
                criteriaBuilder.isMember(otherRoot, root.get(field)),
                otherCriteria.toSpecification().toPredicate(otherRoot, query, criteriaBuilder)
            );
        };
    }
    ...
    protected abstract Criteria<ENTITY> copy();

    protected abstract Specification<ENTITY> toSpecification();
}
